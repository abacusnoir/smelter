...
(handler-case
    (progn
      ...
      ;;; the `match` macro needs ADT accessor functions and type predicates
      ;;; to exist to compile, but those functions are only generated by
      ;;; compiling a `match` expression. We manually define all required
      ;;; functions and type predicates here to ensure they are included
      ;;; in the final executable.
      ;;; ----------------------------------------------------------------

      (format t "Creating comprehensive ADT accessor functions and type predicates...~%")
      ...
      (let ((classes-pkg (find-package :coalton-library/classes)))
        (when classes-pkg
          ...
          (eval `(deftype ,(intern "|tuple/tuple|" classes-pkg) ()
                   '(satisfies ,(intern "TUPLE/TUPLE-TYPE-P" classes-pkg))))
          (eval `(deftype ,(intern "|optional/some|" classes-pkg) ()
                   '(satisfies ,(intern "OPTIONAL/SOME-TYPE-P" classes-pkg))))
          (eval `(deftype ,(intern "|optional/none|" classes-pkg) ()
                   '(satisfies ,(intern "OPTIONAL/NONE-TYPE-P" classes-pkg))))
          ...
          (setf (symbol-function (intern "RESULT/ERR-TYPE-P" classes-pkg))
                (lambda (obj)
                  (and (typep obj 'structure-object)
                       (let ((type-name (type-of obj)))
                         (and (symbolp type-name)
          ...
          (setf (symbol-function (intern "TUPLE/TUPLE-TYPE-P" classes-pkg))
                (lambda (obj)
                  (and (typep obj 'structure-object)
                       (let ((type-name (type-of obj)))
                         (and (symbolp type-name)
                              (string= (symbol-name type-name) "TUPLE/TUPLE")
                              (string= (package-name (symbol-package type-name)) "COALTON-LIBRARY/CLASSES"))))))

          (setf (symbol-function (intern "OPTIONAL/SOME-TYPE-P" classes-pkg))
                (lambda (obj)
                  (and (typep obj 'structure-object)
                       (let ((type-name (type-of obj)))
                         (and (symbolp type-name)
          ...
          (setf (symbol-function (intern "OPTIONAL/NONE-TYPE-P" classes-pkg))
                (lambda (obj)
                  (and (typep obj 'structure-object)
                       (let ((type-name (type-of obj)))
                         (and (symbolp type-name)
          ...
          (setf (symbol-function (intern "|result/err-_0|" classes-pkg))
                (lambda (obj)
                  (slot-value obj (intern "_0" (symbol-package (type-of obj))))))

          (setf (symbol-function (intern "|tuple/tuple-_0|" classes-pkg))
                (lambda (obj)
                  (slot-value obj (intern "_0" (symbol-package (type-of obj))))))

          (setf (symbol-function (intern "|tuple/tuple-_1|" classes-pkg))
                (lambda (obj)
                  (slot-value obj (intern "_1" (symbol-package (type-of obj))))))

          (setf (symbol-function (intern "|optional/some-_0|" classes-pkg))
                (lambda (obj)
                  (slot-value obj (intern "_0" (symbol-package (type-of obj))))))

          (format t "Created comprehensive ADT types and accessors for pattern matching.~%")))

      ;;; Manual Pre-generation of Coalton ADT Accessors (LEGACY - keeping for compatibility)
      ;;; ----------------------------------------------------------------
      ;;; This is the "scalpel" fix for the "chicken-and-egg" problem where
      ;;; the `match` macro needs ADT accessor functions to exist to compile,
      ;;; but those functions are only generated by compiling a `match` expression.
      ;;; In a saved binary, this bootstrap fails. We manually define these
      ;;; functions here to ensure they are included in the final executable,
      ;;; allowing the `match` compiler to work at runtime.

      (format t "Creating manual ADT accessor functions...~%")

      ;; Unlock the package before creating functions
      (sb-ext:unlock-package :coalton-library/classes)

      (let ((classes-pkg (find-package :coalton-library/classes)))
        (when classes-pkg
          ;; Result type predicates and accessors
          (setf (symbol-function (intern "RESULT/OK-P" classes-pkg))
                (lambda (obj)
                  (and (typep obj 'structure-object)
                       (string= (symbol-name (type-of obj)) "OK"))))

          (let ((ok-accessor-sym (intern "|result/ok-_0|" classes-pkg)))
            (setf (symbol-function ok-accessor-sym)
                  (lambda (obj)
                    (slot-value obj (intern "_0" (symbol-package (type-of obj))))))
            (export ok-accessor-sym classes-pkg)
            (format t "Created and exported accessor: ~A~%" ok-accessor-sym))

          (setf (symbol-function (intern "RESULT/ERR-P" classes-pkg))
                (lambda (obj)
                  (and (typep obj 'structure-object)
                       (string= (symbol-name (type-of obj)) "ERR"))))

          (let ((err-accessor-sym (intern "|result/err-_0|" classes-pkg)))
            (setf (symbol-function err-accessor-sym)
                  (lambda (obj)
                    (slot-value obj (intern "_0" (symbol-package (type-of obj))))))
            (export err-accessor-sym classes-pkg)
            (format t "Created and exported accessor: ~A~%" err-accessor-sym))

          ;; Tuple type predicates and accessors
          (setf (symbol-function (intern "TUPLE/TUPLE-P" classes-pkg))
                (lambda (obj)
                  (and (typep obj 'structure-object)
                       (string= (symbol-name (type-of obj)) "TUPLE"))))

          (let ((tuple-accessor-0-sym (intern "|tuple/tuple-_0|" classes-pkg)))
            (setf (symbol-function tuple-accessor-0-sym)
                  (lambda (obj)
                    (slot-value obj (intern "_0" (symbol-package (type-of obj))))))
            (export tuple-accessor-0-sym classes-pkg)
            (format t "Created and exported accessor: ~A~%" tuple-accessor-0-sym))

          (let ((tuple-accessor-1-sym (intern "|tuple/tuple-_1|" classes-pkg)))
            (setf (symbol-function tuple-accessor-1-sym)
                  (lambda (obj)
                    (slot-value obj (intern "_1" (symbol-package (type-of obj))))))
            (export tuple-accessor-1-sym classes-pkg)
            (format t "Created and exported accessor: ~A~%" tuple-accessor-1-sym))

          (format t "Manual ADT accessor functions created successfully.~%")))
